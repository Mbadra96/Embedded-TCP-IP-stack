...
void send_tcp_data(uint16_t len,uint8_t *data,uint8_t flag){
	...
	set_ip_total_length(20+20+len);
	enc28j60PacketSend(len+20+20+14,buffer);
}
void send_udp_data(uint16_t len,uint8_t *data){
	...
	set_ip_total_length(20+8+len);
	enc28j60PacketSend(len+20+8+14,buffer);
}
...
uint8_t receive_data(uint8_t (*check_fnc)(void)){
		...
		while(!flag){
			...
			if(!flag) {
				reset_buffer();
				len=enc28j60PacketReceive(1024,buffer);
				}
		}
		return 1;
}
