...
void init_dhcp_struct(uint8_t *src_mac){
	dhcp_struct.opcode = 1;
	dhcp_struct.hw_type= 1;
	dhcp_struct.hw_add_len = 6;
	dhcp_struct.hop_count =0;
	dhcp_struct.num_secs=0;
	dhcp_struct.flags = 0;
	...
	uint8_t magic_cookie[]= {99,130,83,99};
	for(uint8_t i=0;i<4;i++){
		dhcp_struct.magic_cookie[i]=magic_cookie[i];
	}
}

void init_dhcp_in_buffer(void){
	buffer[DHCP_OPCODE] = dhcp_struct.opcode;
	buffer[DHCP_HW_TYPE] = dhcp_struct.hw_type;
	buffer[DHCP_HW_ADD_LENGTH] =dhcp_struct.hw_add_len;
	buffer[DHCP_HOP_COUNT] =dhcp_struct.hop_count;
	for (uint8_t i = 0;i<4;i++)
	{
		...
		buffer[DHCP_MAGIC_COOKIE+i]= dhcp_struct.magic_cookie[i];
	}
	buffer[DHCP_SECONDS] = (uint8_t)(dhcp_struct.num_secs >>8);
	buffer[DHCP_SECONDS+1] =(uint8_t) dhcp_struct.num_secs;
	buffer[DHCP_BROADCAST_FLAG] = (uint8_t)(dhcp_struct.flags >>8);
	buffer[DHCP_BROADCAST_FLAG+1] = (uint8_t)(dhcp_struct.flags);
